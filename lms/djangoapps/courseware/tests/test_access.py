...
class AccessTestCase(LoginEnrollmentTestCase, ModuleStoreTestCase):
    ...
    def test__has_access_courselike_can_enroll(self):
        yesterday = datetime.datetime.now(pytz.utc) - datetime.timedelta(days=1)
        tomorrow = datetime.datetime.now(pytz.utc) + datetime.timedelta(days=1)
        ...
        self.assertTrue(access._has_access_courselike(user, 'enroll', course))
        ...
        user = UserFactory.create()
        self.assertFalse(access._has_access_courselike(user, 'enroll', course))
    ...
    def test__catalog_visibility(self):
        ...
        self.assertTrue(access._has_access_courselike(user, 'see_in_catalog', course))
        self.assertTrue(access._has_access_courselike(user, 'see_about_page', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_in_catalog', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_about_page', course))
        ...
        self.assertFalse(access._has_access_courselike(user, 'see_in_catalog', course))
        self.assertTrue(access._has_access_courselike(user, 'see_about_page', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_in_catalog', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_about_page', course))
        ...
        self.assertFalse(access._has_access_courselike(user, 'see_in_catalog', course))
        self.assertFalse(access._has_access_courselike(user, 'see_about_page', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_in_catalog', course))
        self.assertTrue(access._has_access_courselike(staff, 'see_about_page', course))
    ...
    def test_access_on_course_with_pre_requisites(self):
        ...
        response = access._has_access_courselike(user, 'view_courseware_with_prerequisites', course)
        self.assertFalse(response)
        self.assertIsInstance(response, access_response.MilestoneError)
        # Staff can always access course
        staff = StaffFactory.create(course_key=course.id)
        self.assertTrue(access._has_access_courselike(staff, 'view_courseware_with_prerequisites', course))

        # User should be able access after completing required course
        fulfill_course_milestone(pre_requisite_course.id, user)
        self.assertTrue(access._has_access_courselike(user, 'view_courseware_with_prerequisites', course))
    ...
    def test__access_on_mobile(self, mobile_available, student_expected, staff_expected):
        ...
        self.assertEqual(
            bool(access._has_access_courselike(self.student, 'load_mobile', descriptor)),
            student_expected
        )
        self.assertEqual(bool(access._has_access_courselike(self.staff, 'load_mobile', descriptor)), staff_expected)
